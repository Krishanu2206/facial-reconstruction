...
class YOLOv8_face:
    ...
    def draw_detections(self, image, boxes, scores, kpts):
        for box, score, kp in zip(boxes, scores, kpts):
            ...
            cv2.putText(image, f"face: {score:.2f}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            for i in range(5):
                cv2.circle(image, (int(kp[i * 3]), int(kp[i * 3 + 1])), 2, (0, 255, 0), thickness=-1)
        return image

def process_video(video_path, face_model):
...
def main():
    ...
    parser.add_argument('--modelpath', type=str, default='models/saved_models/yolov8n-face.onnx', help="onnx filepath")
    parser.add_argument('--confThreshold', default=0.45, type=float, help='class confidence')
    parser.add_argument('--nmsThreshold', default=0.5, type=float, help='nms iou thresh')
    args = parser.parse_args()

    # Initialize YOLOv8_face object detector
    face_model = YOLOv8_face(args.modelpath, conf_thres=args.confThreshold, iou_thres=args.nmsThreshold)
    
    # Process video
    process_video(args.videopath, face_model)
